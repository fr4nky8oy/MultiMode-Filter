...
class TwentyFourDbFilterTest : public juce::UnitTest
{
public:
    TwentyFourDbFilterTest() : juce::UnitTest("24dB Filter Test") {}
    
    void runTest() override
    {
        beginTest("24dB/octave slope verification");
        ...
        // Set filter to 24dB/oct slope
        auto* slopeParam = processor.parameters.getParameter("slope");
        if (slopeParam)
        {
            slopeParam->setValueNotifyingHost(2.0f / 2.0f); // Index 2 = 24dB
        }
        ...
        testFrequencyResponse(processor, 1000.0f, sampleRate, bufferSize, -3.0f, 3.0f);  // At cutoff (-3dB)
        testFrequencyResponse(processor, 2000.0f, sampleRate, bufferSize, -24.0f, 5.0f); // 1 octave above (-24dB)
        testFrequencyResponse(processor, 4000.0f, sampleRate, bufferSize, -48.0f, 6.0f); // 2 octaves above (-48dB)
    }
...
static TwentyFourDbFilterTest twentyFourDbFilterTest;